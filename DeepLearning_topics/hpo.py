...
def test(model, val_loader, criterion):
    ...
    average_accuracy = running_corrects/len(val_loader.dataset)
...
def create_data_loaders(datadir, batch_size):
    ...
    validation_path = os.path.join(datadir, 'valid')
...
def main(args):
    ...
    train_data_loader, val_data_loader = create_data_loaders(datadir=args.data_dir, batch_size=args.batch_size)
    
    ## train model
    model = train(model, train_data_loader, args.epochs, loss_criteria, optimizer)
    ## validation
    test(model, val_data_loader, loss_criteria)
...
if __name__=='__main__':
    ...
    parser.add_argument("--batch_size", type=int, default=64, metavar="N", help="train batch size")
    parser.add_argument( "--test_batch_size", type=int, default=1000, metavar="N", help="test batch size")
